...
while True:
    ...
    if not is_fallback:
        ...
        # Construir el mensaje de alerta de velocidad
        if not unidades_en_alerta_speed.empty:
            ...
            for _, row in unidades_en_alerta_speed.head(5).iterrows():
                ...
                # Resaltar si es exceso crítico (> 74.0)
                estado_critico = "🚨 CRÍTICO" if row['VELOCIDAD'] > 74.0 else "⚠️ ALERTA"
                
                mensaje_alerta_speed += (
                    f"**{nombre_unidad}** ({estado_critico} a {velocidad_formateada}):\n"
                    f"---\n"
                )
    ...
    with alerta_velocidad_placeholder.container():
        if not unidades_en_alerta_speed.empty:
            ...
            # Usar st.error/st.warning/st.info según la gravedad (st.error es ROJO, similar al crítico)
            ...
            if total_alertas_pendientes_speed > 0:
                st.button(
                    ...
                    on_click=lambda: [descartar_alerta_velocidad(uid) for uid in unidades_en_alerta_speed['UNIDAD']],
                    type="secondary"
                )
        else:
        ...
        if not unidades_en_alerta_stop.empty:
            ...
            if total_alertas_pendientes > 0:
                st.button(
                    ...
                    on_click=lambda: [descartar_alerta_stop(uid) for uid in unidades_en_alerta_stop['UNIDAD']],
                    type="secondary"
                )
        else:
